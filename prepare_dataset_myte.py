...
dataset files using the MYTE tokenizer (Transformers-based).

Usage:
    python prepare_dataset_myte.py \
        /path/to/txt/files \
        -o /path/to/output_dir \
        --decompose-map /path/to/decompose_map.json \
        --merge-map /path/to/merge_map.json \
        --eos-token-id 1 \
        --workers 48
"""
...
# NOTE: Adjust path as needed or ensure myte is installed in environment
sys.path.insert(0, "/home/k/kieron7/fyp")
from myt5_tokenizer import MyT5Tokenizer

# --- SETUP RICH CONSOLE & LOGGING ---
# Define a console to share between logging and progress so they don't fight
...
def fill_memmap(
    ...
    tokenizer = MyT5Tokenizer(
        decompose_map=decompose_map_path,
        merge_map=merge_map_path
    )
...
@click.option(
    "--decompose-map-path",
    type=str,
    help="Path to the MYTE decompose map file",
    required=True,
)
@click.option(
    "--merge-map-path",
    type=str,
    help="Path to the MYTE merge map file",
    required=True,
)
@click.option(
    "--eos-token-id",
...
def main(
    ...
    log.info(f"decompose_map:    {decompose_map_path}")
    log.info(f"merge_map:        {merge_map_path}")
    ...
    fill_memmap_fn = functools.partial(
        fill_memmap,
        decompose_map_path=decompose_map_path,
