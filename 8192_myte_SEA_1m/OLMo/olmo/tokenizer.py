...
class Tokenizer:
    ...
    def __init__(
        self,
        base_tokenizer: Union[BaseTokenizer, AutoTokenizer],
        eos_token_id: int,
        use_transformers_tokenizer: bool = True,
        pad_token_id: Optional[int] = None,
        truncate_to: Optional[int] = None,
        truncate_direction: Union[str, TruncationDirection] = TruncationDirection.right,
    ):
        self.base_tokenizer = base_tokenizer
        self.use_transformers_tokenizer = use_transformers_tokenizer
    ...
    def vocab_size(self) -> int:
        if self.use_transformers_tokenizer:
            return len(self.base_tokenizer)
        else:
            return self.base_tokenizer.get_vocab_size(with_added_tokens=True)
    ...
    def from_train_config(cls, config: TrainConfig) -> Tokenizer:
        ...
        tokenizer = cls.from_pretrained(
            ...
            config.tokenizer.use_transformers_tokenizer,
    ...
    @classmethod
    def from_pretrained(cls, identifier: str, use_transformers_tokenizer: bool = True, **kwargs) -> Tokenizer:
        """
        ...
        return cls(base_tokenizer, eos_token_id, use_transformers_tokenizer=use_transformers_tokenizer, **kwargs)
    ...
    def from_checkpoint(cls, checkpoint_dir: PathOrStr) -> Tokenizer:
        ...
        tokenizer = cls.from_pretrained(
            ...
            tokenizer_config.use_transformers_tokenizer,
    ...
    def encode_batch(self, inputs: List[str], add_special_tokens: bool = True) -> List[List[int]]:
        ...
        batch_encoding = self.base_tokenizer(
            inputs,
            add_special_tokens=False,
            truncation=False,
        )
        all_input_ids = []
        for encoding in batch_encoding["input_ids"]:
            encoding_ids = self._truncate(encoding, truncate_to, self.truncate_direction)
            if add_special_tokens:
                encoding_ids = self.add_special_tokens(encoding_ids)
            all_input_ids.append(encoding_ids)
