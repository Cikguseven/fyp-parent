...
# Original OLMo2 GPT-NeoX / GPT-2-style byte-level BPE
...
# Plain byte-level BPE (non-parity-aware variant)
"byte_bpe": {"eos_token_id": 90369, "vocab_size": 90372},
# Parity-Aware BPE — same vocab layout as byte_bpe
"parity_bpe": {"eos_token_id": 90369, "vocab_size": 90372},
}
...
def detect_tokenizer_type(tokenizer_config: dict) -> str:
    ...
    # 1. Fast heuristics on the identifier string
    if "myte" in id_lower or "myt5" in id_lower:
        return "myte"
    if "parity" in id_lower:
        return "parity_bpe"

    # 2. Inspect tokenizer_config.json for explicit class names
    ...
    if tok_cfg_path.is_file():
        ...
        if "MyT5" in tok_class or "MyT5" in str(auto_map) or "myte" in tok_class.lower():
            return "myte"
        if "Parity" in tok_class or "parity" in tok_class.lower():
            return "parity_bpe"

        # 3. Distinguish byte_bpe from gpt2 by inspecting tokenizer.json
        tok_json_path = Path(identifier) / "tokenizer.json"
        if tok_json_path.is_file():
            with open(tok_json_path) as f:
                tok_json = json.load(f)
            pre_tok_type = tok_json.get("pre_tokenizer", {}).get("type", "")
            # A ByteLevel pre-tokenizer that is NOT the canonical GPT-2/NeoX vocab
            # is treated as byte_bpe.
            if pre_tok_type == "ByteLevel" and "gpt" not in id_lower and "neox" not in id_lower:
                return "byte_bpe"
...
def _write_tokenizer_gpt2(
    ...
    """Write a GPT-2-style byte-level BPE tokenizer (original OLMo2 behaviour)."""
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'byte_bpe'/'parity_bpe' use eos_token_id=90369, vocab_size=90372. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(50279 for gpt2, 1 for myte, 90369 for byte_bpe/parity_bpe)."
        ),
    )
    parser.add_argument(
        "--no_tmp_cleanup",
